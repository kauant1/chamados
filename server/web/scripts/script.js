...
createButton(button_reload, 'Voltar', '8px')
...
select1.addEventListener('change', () =>{
    ...
    if (select1.value !== "") {
        select2.removeAttribute('disabled'); // Use removeAttribute for enabling
        text1.style.color = 'green';
      } else {
        select2.setAttribute('disabled', 'true'); // Re-disable if select1 is empty
...
select2.addEventListener('change', () =>{
    ...
    if (select2.value !== "") {
        input1.removeAttribute('disabled'); // Use removeAttribute for enabling
        text2.style.color = 'green';
      } else {
        input1.setAttribute('disabled', 'true'); // Re-disable if select1 is empty
...
button_send.addEventListener ('click', () =>{
  ...
  const pass = document.querySelector(".login_password").value
  alert(`user: ${user} \n pass: ${pass}`)
